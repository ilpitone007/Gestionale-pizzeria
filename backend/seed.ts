...
async function main() {
  ...
  // Operatori seed
  const operatori = [
    { nome: 'Admin', cognome: 'Pizzeria', pin: await bcrypt.hash('0000', 10), ruolo: 'admin' },
    { nome: 'Luca', cognome: 'Bianchi', pin: await bcrypt.hash('1234', 10), ruolo: 'operatore' },
    { nome: 'Sara', cognome: 'Verdi', pin: await bcrypt.hash('5678', 10), ruolo: 'operatore' },
    { nome: 'Marco', cognome: 'Rossi', pin: null, ruolo: 'operatore' }
  ];

  for (const op of operatori) {
    const existing = await prisma.operatore.findFirst({
      where: { nome: op.nome, cognome: op.cognome }
    });
    if (existing) {
