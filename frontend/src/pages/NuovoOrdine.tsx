...
export default function NuovoOrdine() {
  ...
  const { operatore } = useOperatoreStore();
  ...
  return (
    <div className="flex h-full flex-col md:flex-row relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {pizzeFiltrate.map(pizza => (
            ...
            >
              ...
              <button
                className="mt-4 w-full bg-red-50 text-red-600 font-medium py-2 rounded-lg hover:bg-red-100 flex justify-center items-center gap-2 min-h-[44px]"
      ...
      <div className="md:hidden fixed bottom-[56px] left-0 right-0 p-4 bg-white border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-40">
        <button
          className="w-full bg-red-600 text-white font-bold py-3 rounded-lg flex justify-between items-center px-4"
          onClick={() => setIsCartOpenMobile(!isCartOpenMobile)}
        >
          <span className="flex items-center gap-2"><ShoppingCart className="w-5 h-5" /> Vedi Ordine ({voci.length})</span>
          <span className="flex items-center gap-2">€{calculateTotal().toFixed(2)} {isCartOpenMobile ? <ChevronDown /> : <ChevronUp />}</span>
        </button>
      </div>

      {/* Cart Container (Drawer on mobile, Sidebar on desktop) */}
      <div className={`fixed inset-x-0 bottom-[128px] md:bottom-0 md:relative md:inset-auto md:w-96 bg-white md:border-l flex flex-col shadow-2xl md:shadow-lg transition-transform duration-300 z-30 ${isCartOpenMobile ? 'translate-y-0 h-[65vh] rounded-t-2xl' : 'translate-y-full md:translate-y-0 h-0 md:h-full overflow-hidden'}`}>
        ...
        <div className="p-4 border-b space-y-3 bg-white">
          <div>
            ...
            <input
              type="text"
              className="w-full border rounded p-3 text-base focus:ring-2 focus:ring-red-500 focus:outline-none"
              value={nomeCliente}
              ...
              return (
                <div key={voce.id} className="bg-white p-3 rounded-lg border shadow-sm">
                  ...
                  <button
                    ...
                    className="text-xs text-red-600 hover:text-red-800 flex items-center gap-1 mt-2 min-h-[32px] px-2 -ml-2"
                  >
        ...
        <div className="p-4 border-t bg-white">
          ...
          <button
            ...
            className="w-full bg-red-600 text-white font-bold py-4 rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Conferma Ordine
          </button>
        </div>
      ...
      {/* Customize Modal */}
      {pizzaToCustomize && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end md:items-center justify-center z-[60] pb-[env(safe-area-inset-bottom)]">
          <div className="bg-white rounded-t-2xl md:rounded-xl shadow-2xl w-full max-w-lg flex flex-col h-[85vh] md:h-auto md:max-h-[90vh]">
            ...
            <div className="p-4 overflow-auto flex-1">
                ...
                <div key={cat.id} className="mb-6">
                  ...
                  <div className="space-y-2">
                    {cat.aggiunte.map((agg: any) => {
                      ...
                      return (
                        <label key={agg.id} className={`flex items-center justify-between p-3 rounded-lg cursor-pointer border transition-colors ${isSelected ? 'bg-red-50 border-red-200' : 'hover:bg-gray-50 border-gray-200'}`}>
              ...
              <div className="mt-4">
                ...
                <textarea
                  className="w-full border rounded-lg p-3 text-base focus:ring-2 focus:ring-red-500 focus:outline-none"
                  rows={2}
                  value={tempNote}
            ...
            <div className="p-4 border-t bg-gray-50 rounded-b-xl flex justify-between items-center">
              ...
              <button
                ...
                className="bg-red-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-red-700 min-h-[44px]"
              >
                Aggiungi all'ordine
              </button>
            </div>
